...
from apps.processor.services.whisper_service import WhisperAlignmentService
...
def process_video_render_task(self, project_id):
    ...
    try:
        with transaction.atomic():
            ...
            for idx, chunk in enumerate(aligned_timeline):
                ...
                video_path = fetch_background_video(keyword, project_id=project.human_name, chunk_index=idx)
                video_clips.append((video_path, duration))
                ...
                logger.info(f"Successfully fetched and registered video clip: {video_path}")
            ...
            if audio_path and video_clips:
                try:
                    ...
                    sum_prior_durations = sum(clip[1] for clip in video_clips[:-1])
                    new_last_duration = max(0.1, real_audio_duration - sum_prior_durations)
                    
                    last_video_path, old_last_duration = video_clips[-1]
                    video_clips[-1] = (last_video_path, new_last_duration)
                    logger.info(
                        f"Adjusted last video clip duration from {old_last_duration:.3f}s to {new_last_duration:.3f}s "
                        f"to match real audio duration {real_audio_duration:.3f}s."
                    )
    ...
    # 4. Assemble the final video using FFmpeg with Whisper-exact durations
    ...
    try:
        ...
        assemble_final_video(audio_path, video_clips, final_output_path)
